...
@testset "EDSolver" begin
    ...
    solver = EDSolver(EDKind(hilbert), Partition(:N, table, bs), refergenerator, bs, table; m=150)
    ...
    referterms = (t, s, μ)
    refergenerator = OperatorGenerator(referterms, referbonds, hilbert; table = table)
    solver = EDSolver(EDKind(hilbert), Partition(:A, table, bs), refergenerator, bs, table; m=150)
