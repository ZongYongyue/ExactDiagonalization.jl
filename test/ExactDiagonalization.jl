...
@testset "finiteNormalGreenFunction" begin
    unitcell = Lattice([0, 0]; vectors=[[1, 0]])
    cluster = Lattice(unitcell, (2,), ('p',))
    hilbert = Hilbert(site=>Fock{:f}(1, 2) for site=1:length(cluster))
    bs = BinaryBases(4, 2)
